...
// --- Автоматическая подгрузка заявок при загрузке страницы и при переходе на вкладку 'Анкеты' ---
...
async function addForm(formData) {
    try {
        const res = await fetch('/api/forms', {
            ...
            body: JSON.stringify({ role: formData.role, data: formData })
        });
        if (!res.ok) throw new Error('Ошибка добавления заявки');
        await loadForms();
    } catch (err) {
